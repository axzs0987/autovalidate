import os
import json
from cnn_model import CNNnet, CNNnet_Cosine,CNNnetTriplet,CNNRCModel
import torch
import random
...
class TripletSelection():
    def __init__(self, batch_size, inner_times, outer_times, topk, max_inclass, margin, is_test=False):
        self.margin=margin
        ...
        self.auchors = []
        self.triplet_pair = []
        self.training_data = self.load_data(is_test)
        self.content_dict = {}
    ...
    def get_feature_vector(self,feature, is_test=False):
        ...
        if is_test == True:
            with open("json_data/rc_content_dict_1.json", 'w') as f:
                json.dump(self.content_dict, f)
            with open("json_data/rc_content_temp_dict_1.json", 'w') as f:
                json.dump(self.content_tem_dict, f)
        else:
            with open("json_data/rc_content_dict.json", 'w') as f:
                json.dump(self.content_dict, f)
            with open("json_data/rc_content_temp_dict.json", 'w') as f:
                json.dump(self.content_tem_dict, f)
        return result
    ...
    def load_data(self, is_test=False):
        if is_test:
            if os.path.exists('json_data/rc_content_dict_1.json'):
                with open("json_data/rc_content_dict_1.json", 'r') as f:
                    self.content_dict = json.load(f)
            if os.path.exists('json_data/rc_content_temp_dict_1.json'):
                with open("json_data/rc_content_temp_dict_1.json", 'r') as f:
                    self.content_tem_dict = json.load(f)
        else:
            log_print = open('rc_training_out_margin20.log', 'w')
            sys.stdout = log_print
            sys.stderr = log_print
            if os.path.exists('json_data/rc_content_dict.json'):
                with open("json_data/rc_content_dict.json", 'r') as f:
                    self.content_dict = json.load(f)
            if os.path.exists('json_data/rc_content_temp_dict.json'):
                with open("json_data/rc_content_temp_dict.json", 'r') as f:
                    self.content_tem_dict = json.load(f)
        ...
        if os.path.exists("json_data/100000_all_list.json"):
            ...
            filelist = os.listdir("../../data/sheet_features/")
    ...
    def transfer_origin_feature(self,origin_feature):
        ...
        feature = self.get_feature_vector(feature)
        return feature
    ...
    def inner_training(self, time):
        ...
        for epoch in range(self.inner_times):
            ...
            torch.save(self.model,'rc_cnn_dynamic_triplet_margin_'+str(self.margin)+"_"+str(time)+"_"+str(epoch))
        gc.collect()
        del self.features
        del train_data
        del train_loader
    ...
    def batch_get(self, origin_feature_list, embed_features, id2filesheet, lock):
        ...
        for item in origin_feature_list:
            ...
            feature = feature.reshape(1,100,10,11).permute(0,3,1,2)
    ...
    def load_embed_features(self):
        ...
        for i in self.mini_batch:
            ...
            if len(temp_batch) >= batch_len:
                ...
                temp_batch = []
        self.batch_get_single(temp_batch, self.embed_features, self.id2filesheet)
        # procs.append(multiprocessing.Process(target=self.batch_get, args=(temp_batch, embed_features, id2filesheet, lock), callback=callback,error_callback=errorcallback))
        # res = pool.apply_async(self.batch_get,args=(temp_batch, embed_features, id2filesheet, lock,),callback=callback,error_callback=errorcallback)
        temp_batch = []
    ...
    def get_mini_batch(self, size=1400):
        ...
        while len(self.mini_batch) < size:
            ...
            # print(sheetname)
            if not os.path.exists('../../data/sheet_features/'+filename.split('/')[-1]+"---"+sheetname+'.json'):
            ...
            for temp_incalss_index in range(self.inclass_index+1, min(min(len(self.sheetname_2_file_devided[sheetname][self.class_index]['filenames']),self.inclass_index+size-len(self.mini_batch)+1), self.max_inclass)):
                ...
                if os.path.exists('../../data/sheet_features/'+self.sheetname_2_file_devided[sheetname][self.class_index]['filenames'][temp_incalss_index].split('/')[-1]+"---"+sheetname+'.json'):
    ...
    def get_negative_case(self, data, sorted_list_index, sorted_list_dict, res, lock):
        ...
        # print(self.id2filesheet)
            ...
            print('distance', pos_distance, neg_distance)
    ...
    def get_mini_batch_triplets(self):
        ...
        np.save('rc_triplet_pair.npy', self.triplet_pair)
        # for index,triplet in enumerate(self.triplet_pair):
        #     print(index, triplet)
        #     if len(triplet) == 3:
        #         continue
    ...
    def test_on_one_model(self, model_path, test_data):
        ...
        for test_x in test_loader:
            ...
            # print(test_x[0].shape)
            # print(test_x[:,0,:].reshape(len(test_x),100,10,11))
            test_x1 =test_x[:,0,:].reshape(len(test_x),100,10,11).permute(0,3,1,2)
            test_x2 = test_x[:,1, :].reshape(len(test_x),100,10,11).permute(0,3,1,2)
            test_x3 =  test_x[:,2,:].reshape(len(test_x),100,10,11).permute(0,3,1,2)
    ...
    def split_data(self):
        ...
        with open("json_data/rc_dynamic_triplet_features.json",'r') as f:
            triplet_features = json.load(f)
        with open("json_data/rc_dynamic_triplet_id.json",'r') as f:
        ...
        with open("rc_dynamic_shuffle_triplet_fature.json", 'w') as f:
            json.dump(triplet_features, f)

        with open("rc_dynamic_cnn_triplet_x_1.json", 'w') as f:
            json.dump(x_1, f)
        with open("rc_dynamic_cnn_triplet_x_2.json", 'w') as f:
            json.dump(x_2, f)
        with open("rc_dynamic_cnn_triplet_x_3.json", 'w') as f:
            json.dump(x_3, f)
        with open("rc_dynamic_cnn_triplet_x_4.json", 'w') as f:
            json.dump(x_4, f)

        with open("rc_dynamic_cnn_triplet_index_1.json", 'w') as f:
            json.dump(index1, f)
        with open("rc_dynamic_cnn_triplet_index_2.json", 'w') as f:
            json.dump(index2, f)
        with open("rc_dynamic_cnn_triplet_index_3.json", 'w') as f:
            json.dump(index3, f)
        with open("rc_dynamic_cnn_triplet_index_4.json", 'w') as f:
            json.dump(index4, f)
    ...
    # with open("../AnalyzeDV/CNN_training_origin_dict_filter.json", 'r', encoding='utf-8') as f:
        ...
        for cnn_feature in [cnn_feature_2, cnn_feature_3]:
            for sheetname in cnn_feature.keys():
                ...
                for item in cnn_feature[sheetname]:
                    ...
                    id_2_feature[id_] = self.get_feature_vector(feature, is_test=True)
        ...
        with open("json_data/rc_dynamic_cnn_features_origin_dict.json", 'w') as f:
            json.dump(result, f)
        with open("json_data/rc_dynamic_sheet_2_id.json", 'w') as f:
            json.dump(sheet_2_id, f)
        with open("json_data/rc_dynamic_id_2_feature.json", 'w') as f:
            json.dump(id_2_feature, f)

    def get_triplet_feature(self):
        with open("json_data/triplet_pair.json", 'r') as f:
            triplet_pair = json.load(f)
        with open("json_data/rc_dynamic_id_2_feature.json", 'r') as f:
            id_2_feature = json.load(f)
        with open("json_data/rc_dynamic_sheet_2_id.json", 'r') as f:
        ...
        print(positive_num)
        with open("json_data/rc_dynamic_triplet_features.json",'w') as f:
            json.dump(res, f)
        with open("json_data/rc_dynamic_triplet_id.json",'w') as f:
            json.dump(id_list, f)
    def batch_testing(self):
        ...
        #     triplet_features = json.load(f)
        with open("rc_dynamic_cnn_triplet_x_1.json", 'r') as f:
            x1 = json.load(f)
        with open("rc_dynamic_cnn_triplet_x_2.json", 'r') as f:
            x2 = json.load(f)
        with open("rc_dynamic_cnn_triplet_x_3.json", 'r') as f:
            x3 = json.load(f)
        with open("rc_dynamic_cnn_triplet_x_4.json", 'r') as f:
            x4 = json.load(f)
        ...
        for i in range(5,19):
            for j in range(0,2):
                model_path = "rc_cnn_dynamic_triplet_margin_20_" + str(i) + "_" + str(j)
        ...
        # for model_path in model_pathes:
            ...
            #     x_train_list.append(x_train_sub)
                ...
                np.save("rc_distance1_"+str(i) + '_' + str(j) +'.npy', distance1)
                np.save("rc_distance2_"+str(i) + '_' + str(j) +'.npy', distance2)
...
def get_roc_curve():
    ...
    for epoch in range(0,2):
        for time in range(0,19):
            y_triplet_score = []
            y_triplet_test = []
            distance1 = np.load('rc_distance1_'+str(time)+'_'+ str(epoch)+'.npy', allow_pickle=True)
            distance2 = np.load('rc_distance2_'+str(time)+'_'+ str(epoch)+'.npy', allow_pickle=True)
            for distance in distance1:
                y_triplet_score.append(0-distance)
                y_triplet_test.append(1)
    ...
    plt.savefig("rc_triplet_dymatic_loss_roc.png")
            
def get_threshold_p_r():
    y_triplet_score = []
    y_triplet_test = []
    y_triplet_pred = []
    distance1 = np.load('rc_distance1_'+str(1)+'_'+ str(1)+'.npy', allow_pickle=True)
    distance2 = np.load('rc_distance2_'+str(1)+'_'+ str(1)+'.npy', allow_pickle=True)
    for distance in distance1:
        y_triplet_score.append(0-distance)
        y_triplet_test.append(1)
...
if __name__ == '__main__':
    # triplet_selection = TripletSelection(batch_size=128, inner_times=2, outer_times=20, topk=30, max_inclass=40, margin=20)
    # triplet_selection.outer_training()
    triplet_selection = TripletSelection(batch_size=128, inner_times=2, outer_times=20, topk=30, max_inclass=40, margin=20, is_test=True)
